...
func NewConsumer(logger *zap.Logger, kconsumer *kafka.Consumer, config *ConsumerConfig, handler Handler) (Consumer, error) {
	cfg := &ConsumerConfig{
		Topic:              "",
		MaxRetries:         5,
	...
	if cfg.Topic == "" {
		return nil, errors.New("consumer topic can't be empty")
	}
