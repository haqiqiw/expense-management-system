...
describe('ApprovalListPage', () => {
  ...
  it('render loading message', async () => {
    ...
    const wrapper = mount(ApprovalListPage, {
      global: {
        stubs: {
  ...
  it('render table with expense data', async () => {
    ...
    expect(wrapper.html()).toContain('Pengeluaran Butuh Persetujuan')
  ...
  it('call the setPage action when next button is clicked', async () => {
    mockedApi.get.mockResolvedValue({
      data: {
        data: [mockExpense],
        meta: {
    ...
    const nextButton = wrapper.findAll('#btn-next')[0]
